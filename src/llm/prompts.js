...
另外（T-438 §3，**在同一份 JSON 里多给一个 cast 键**）：
从角色卡与上面这些世界书资料里，把**有明确人物证据的候选角色**列出来（不限制人数）：
- 只列真的会出场的人物：有人称、有行为、有对话，或有独立段落描写的
- 不列栏目名（性格/外貌/年龄/代号这类标签名）、组织、地点、抽象名词
- 花名 / 别名 / 代号的值放进 aliases（例：\`代号：Lobo\` → name 是本人、aliases 收 Lobo）
- 证据越充分 confidence 越高（0~1）；拿不准就给低分，不要硬凑
- 用户另外指定了这些角色：{{knownCast}}。资料里有对应描写的，列进 cast 并给出 evidence；
  **资料里找不到的也要保留**（这是用户明确点名的角色），confidence 给 0.5 左右
- 只多出这一个键，其余八项照上面写：
  "cast": [{ "name": "裴玉", "aliases": ["Lobo"], "confidence": 0.9, "evidence": "原文短摘录" }]`,
  },
