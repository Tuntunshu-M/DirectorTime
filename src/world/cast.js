// 导演时间 · 主角设置（T-412 多人卡适配 + T-437 世界书识别 + T-438 收尾）
//
// 多人卡场景里，一局可能有几个"主角"（用户想跟着谁走）。现在两种来源：
//   · **世界书识别**：从当前已勾选的世界书条目正文里抽出来的角色（T-437，本地零调用）
//   · **自选**：手动填名字（`manual: true`）—— 给"想攻略的 NPC / 识别漏了的"用
//
// ⚠️ 2026-09-15（T-438 §6）：**「酒馆里的角色卡列表」那一节已按维护者要求整块删除**
//    （`ctx.listCharacters()` / `castApi.candidates` / 界面那一节都没了）。
//    `toggleProtagonist`（按 id 勾选卡）保留为纯函数以备复用，但 src 里已无人调用。
//
// 三件事：
