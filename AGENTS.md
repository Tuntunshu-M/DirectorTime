...
任务清单与验收判据由维护者在本地维护，不在本仓库。接活时向维护者索取对应任务的判据，
**一次只做一个任务**。
...
**`src/ui/settings.js` 是测试用临时界面，不是最终 UI。** 正式界面尚未实现（规划为六分类：事件 / 剧本 / 人物 / 世界书 / 偏好 / 副本，牛皮纸工作台风格）。别在 settings.js 上做美化。
